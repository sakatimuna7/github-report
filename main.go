...
func main() {
	if envB != "" {
		...
		for k, v := range m { os.Setenv(k, v) }
	}
	_ = godotenv.Load()
	h, _ := os.UserHomeDir()
	if h != "" { _ = godotenv.Load(h + "/.ghreport") }
	...
	if *branch == "" { *branch = sh("git", "rev-parse", "--abbrev-ref", "HEAD") }
	if *tok == "" { *tok = sh("gh", "auth", "token") }
	...
	*mod = sel("AI Model", []string{"gemini-flash", "gemini-flash-lite", "groq-llama", "groq-mixtral", "groq-gpt"})
	...
	var d []string
	now := time.Now()
	for i := 0; i < 7; i++ { d = append(d, now.AddDate(0, 0, -i).Format("02/01/2006")) }
	...
	if dr == "Custom Range" {
		...
		v, _ := p.Run(); s, _ = time.Parse("2006-01-02", v)
		p.Label = "Until"; v, _ = p.Run()
		if v != "" { u, _ = time.Parse("2006-01-02", v) }
	} else {
	...
	spin.Suffix = " Fetching..."; spin.Start()
	...
	call := func(m, sp, d string) (string, error) {
		...
		if strings.HasPrefix(m, "gemini") {
			id := "gemini-2.5-flash"
			if m != "gemini-flash" { id = "gemini-2.5-flash-lite-preview-06-17" }
			res, use, err = ai.NewGeminiClient(*gm).GenerateReport(c, id, sp, d)
		} else {
			id := "llama-3.1-8b-instant"
			if m == "groq-mixtral" { id = "mixtral-8x7b-32768" } else if m == "groq-gpt" { id = "openai/gpt-oss-20b" }
	...
	fb := func(pref, sp, d string) (string, error) {
		...
		for _, m := range []string{"gemini-flash", "gemini-flash-lite", "groq-llama"} {
			if m != pref {
				if res, err := call(m, sp, d); err == nil { return res, nil }
			}
		}
	...
	spin.Suffix = " REDUCE..."; spin.Restart()
